...
本项目在开发过程中使用 MCPlato 作为 AI Native Partner 辅助完成需求拆解、阶段规划、代码实现、调试验证和 README 编写。
...
本项目开发过程中使用 MCPlato 作为 AI Native Partner 进行辅助，包括：

- 拆解面试题目要求，形成阶段化开发路线；
- 设计 Agent Loop、工具链和模块化目录结构；
- 辅助实现文本处理、表格处理和 Web UI 功能；
- 通过命令行测试、编译检查和回归验证定位问题；
- 协助整理 README、使用说明和项目亮点表达。

MCPlato 在这个项目中不是运行时依赖，最终项目仍然可以作为独立的本地 Python / Gradio 应用运行。
